...
class ProfileProvider extends UserScopedNotifier {
  ProfileProvider(this.repository, AuthProvider auth) {
  ...
  void bindUser(String uid) {
    watch(repository.watch(uid), (value) => profile = value);
  }
  ...
  Future<bool> save(String name, String bio) => run(() async {
        ...
        if (bio.trim().length > 240) {
          throw const AppException('Keep your bio under 240 characters.');
        }
        await repository.save(
          uid,
          UserProfile(displayName: name.trim(), bio: bio.trim()),
        );
      });
}
